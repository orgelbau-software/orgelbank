...
if ($_POST && isset($_POST['loginmodule'])) {
    ...
    $_SESSION['user']['2fastatus'] = "";
}

if ($_POST && isset($_POST['loginmodule_2fa'], $_POST['otp'], $_SESSION['user']['benutzername'])) {
    if(BenutzerUtilities::validateOTP($_SESSION['user']['benutzername'], $_POST['otp'], true)) {
        $_SESSION['user']['2fastatus'] = "ok";
    } else {
        $_SESSION['user']['2fastatus'] = "";
        $tplStatus = new HTMLStatus("OTP falsch.", HTMLStatus::$STATUS_ERROR);
    }
}
...
if (isset($_SESSION['user']['benutzername'], $_SESSION['user']['passwort'])) {
    ...
    $webUser->setZweiFaktorStatus($_SESSION['user']['2fastatus']);
...
if ($webUser != null && $webUser->passwordLogin() && ! $webUser->isLoginExpired()) {

    if($webUser->isNew2FARequired() ) {
        $tpl2FADialog = new Template("indexkopf_notloggedin_2fa.tpl");
        
        if($tplStatus != null) {
            $tpl2FADialog->replace("Statusmeldung", $tplStatus->getOutput());
        } else {
            $tpl2FADialog->replace("Statusmeldung", "");
        }
        $tplKopf->replace("LoginContent", $tpl2FADialog->forceOutput());
        $tplKopf->anzeigen();
    ...
    $tplLoginDialog = new Template("indexkopf_notloggedin_passwordlogin.tpl");
    if ($_POST) {
    ...
    if ($tplStatus != null)
        $tplLoginDialog->replace("Statusmeldung", $tplStatus->getOutput());
    $tplLoginDialog->replace("Statusmeldung", "");

    $tplKopf->replace("LoginContent", $tplLoginDialog->getOutput());
