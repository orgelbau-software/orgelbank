...
class WartungsListeExcel implements GetRequestHandler
{
    ...
    public function executeGet()
    {
        ...
        $worksheet = $workbook->addWorksheet("Wartungsliste");
        ...
        $cOrgeln = OrgelUtilities::getOrgelListeAnstehendeWartungen($handler['SQLADD']);
        $stimmungen = Constant::getStimmung();

        Log::debug("wartungen anzahl=" . $cOrgeln->getSize());
        if ($cOrgeln->getSize() == 0) {
            $tpl = new HTMLFehlerseite("Ihre Auswahl enth&auml;lt keine Wartungen");
        ...
        if ($cOrgeln != null) {
            foreach ($cOrgeln as $orgel) {
                ...
                if($user->isAdmin()) {
                    $worksheet->write("P" . $iZeile, $orgel->getKostenHauptstimmung(false));
        ...
        foreach ($worksheet->getActiveSheet()->getColumnIterator() as $column) {
            $worksheet->getActiveSheet()->getColumnDimension($column->getColumnIndex())->setAutoSize(true);
        }

        $worksheet->getActiveSheet()->getStyle("P")->getNumberFormat()->setFormatCode(PhpOffice\PhpSpreadsheet\Style\NumberFormat::FORMAT_CURRENCY_EUR);
        $worksheet->getActiveSheet()->getStyle("Q")->getNumberFormat()->setFormatCode(PhpOffice\PhpSpreadsheet\Style\NumberFormat::FORMAT_CURRENCY_EUR);

        $workbook->download("WartungsList-" . date("Ymd_Hi") . ".xlsx");
        $workbook->close();
