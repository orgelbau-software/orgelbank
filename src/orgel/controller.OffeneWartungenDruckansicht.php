...
class OffeneWartungenDruckansicht implements GetRequestHandler
{
    ...
    {
        $tplOrgelDruck = new Template("orgel_wartungen_liste_druck.tpl");
        $tplOrgelDruckDs = new BufferedTemplate("orgel_wartungen_liste_druck_ds.tpl");
        ...
        $c = OrgelUtilities::getOrgelListeAnstehendeWartungen($handler['SQLADD']);
        $tplOrgelDruck->replace("Zyklus", $handler['zyklus']);
        
        $tplOrgelDruck->replace("OrgelAnzahl", $c->getSize());
        ...
        foreach ($c as $oOrgel) {
            ...
            if ($oOrgel->getLetztePflege() == "")
                $tplOrgelDruckDs->replace("NaechstePflege", "&nbsp;");
            $tplOrgelDruckDs->replace("NaechstePflege", $oOrgel->getNaechstePflege(true));
